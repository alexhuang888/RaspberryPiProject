...
void CLaneFinder::FindResponses(IplImage *pImage, int startX, int endX, int y, std::vector<int>& list)
{
...
void CLaneFinder::ProcessSide(std::vector<CLaneInfo> lanes, IplImage *pEdges, bool bRightLane)
{
...
void CLaneFinder::ProcessLanes(CvSeq* lines, IplImage* pEdges, IplImage *pWorkingImage, bool bShowHoughLine)
{
...
int32_t CLaneFinder::ProcessFrame(IplImage *pFrame, float &fAngle, CvPoint &vanishingPoint)
{
	int32_t nRet = -1;
	double rho = 1;
