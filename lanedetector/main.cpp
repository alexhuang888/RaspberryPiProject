...
void processLanes(CvSeq* lines, IplImage* edges, IplImage* temp_frame, const CvFont *pFont, bool bShowHoughLine) 
{
	...
	rt.x = temp_frame->width * 0.65f;//(rt.y - laneR.b.get()) / laneR.k.get();//temp_frame->width * 0.65f;
	rt.y = laneR.k.get() * rt.x + laneR.b.get();
	
	rb.x = temp_frame->width;//(rb.y - laneR.b.get()) / laneR.k.get();//temp_frame->width;
	rb.y = laneR.k.get() * rb.x + laneR.b.get();
	
	lt.x = (rt.y - laneL.b.get()) / laneL.k.get();//temp_frame->width * 0;
	lt.y = rt.y;//laneL.k.get() * lt.x + laneL.b.get();
	
	lb.x = (rb.y - laneL.b.get()) / laneL.k.get();//temp_frame->width * 0.45f;
	lb.y = rb.y;//laneL.k.get() * lb.x + laneL.b.get();
	
	cvLine(temp_frame, rt, rb, CV_RGB(255, 0, 255), 2);
	cvLine(temp_frame, lt, lb, CV_RGB(255, 0, 255), 2);
	cvLine(temp_frame, lt, rt, CV_RGB(128, 0, 128), 2);
	cvLine(temp_frame, lb, rb, CV_RGB(128, 0, 128), 2);	
	
	CvPoint vanishingPoint;
	
	vanishingPoint.x = -(laneR.b.get() - laneL.b.get()) / (laneR.k.get() - laneL.k.get());	// x coordinate
	vanishingPoint.y = laneR.k.get() * vanishingPoint.x + laneR.b.get();
	
	cvLine(temp_frame, cvPoint(vanishingPoint.x, 0), 
		cvPoint(vanishingPoint.x, temp_frame->height), CV_RGB(255, 255, 255), 2);
	...
	cvLine(temp_frame, pivotT, pivotB, CV_RGB(0, 255, 255), 2);	
	
	// find the angle

	float fAngle = 90 - atan2(pivotB.y - pivotT.y, pivotB.x - pivotT.x) * 180 / CV_PI;
	printf("Angle: %f\n", fAngle);
	char szOut[100];
	
	sprintf(szOut, "Angle: %4.2f", fAngle);
	cvPutText(temp_frame, szOut, pivotT, pFont, CV_RGB(0, 255, 255));
}
...
int main(int argc, char* argv[])
{
	...
	while(key_pressed != 27) 
	{
		...
		processLanes(lines, edges, temp_frame, &font, bShowHoughLine);
#if TRACK_CAR		
		// process vehicles
